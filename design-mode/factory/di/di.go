...
type Container struct {
	// (提供者) 假设一种类型只能有一个 provider (供应商) 提供
...
func (c *Container) Provide(constructor interface{}) error {
	...
	/** 获取参数   NumIn()返回func类型的参数个数，如果不是函数，将会panic
	In 返回func类型的第i个参数的类型，如非函数或者i不在[0, NumIn())内将会panic
	reflect.ValueOf().Type().NumIn()
	 */
